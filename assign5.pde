...
void initClocks(){
  ...
  for(int i = 0; i < clockX.length; i++){
    ...
    while(clockY[i] == cabbageY[i] && clockX[i] == cabbageX[i]){
      clockX[i] = SOIL_SIZE * floor(random(SOIL_COL_COUNT));
    }
  }
}
...
void draw() {
	...
	switch (gameState) {
      ...
      if(playerHealth < PLAYER_MAX_HEALTH && isHit(cabbageX[i], cabbageY[i], playerX, playerY, SOIL_SIZE)){
				playerHealth ++;
    ...
    for(int i = 0; i < clockX.length; i++){
      ...
      if(isHit(clockX[i], clockY[i], playerX, playerY, SOIL_SIZE)) { 
		...
		for(int i = 0; i < soldierX.length; i++){
			...
			// Requirement #3: Use boolean isHit(...) to detect collision
			if(isHit(soldierX[i], soldierY[i],  playerX, playerY, SOIL_SIZE)){ 
...
void addTime(float seconds){					// Requirement #2
  gameTimer += seconds * 60.0;
}

boolean isHit(float ax, float ay, float bx, float by, float size){
	if(ax + size > bx    // r1 right edge past r2 left
      && ax < bx + size    // r1 left edge past r2 right
      && ay < by + size // r1 top edge past r2 bottom
      && ay + size > by){
        return true;
      }
  else{
...
String convertFramesToTimeString(int frames){	// Requirement #4
  int second = floor(frames / 60);
  int minute = floor(second / 60);
  second %= 60;
...
void drawCaution(){	// Requirement #6
  ...
  int index = getEnemyIndexByRow(playerRow + 5);
  if(index != -1){
    image(caution, soldierX[index], soldierY[index] - SOIL_SIZE);
  }	
	// HINT:
